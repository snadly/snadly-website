# Kilo — product definition

> **Internal reference.** What Kilo actually does and how it works. For anyone writing or editing site copy.
...
3. **Provisions the stack** — for clients without a data warehouse, Kilo sets up Snowflake and dbt Core
...
- **Not Tableau.** Tableau is a visual analytics layer. Kilo generates the warehouse and models that sit underneath. The output can feed Tableau (or any other BI tool), but Kilo itself is not trying to be one.
...
2. Kilo interprets source schemas and generates:
...
3. If the client does not have a warehouse, Kilo provisions Snowflake and dbt Core
...
|---|---|---|
| Warehouse | Snowflake | Provisioned by Kilo if client has none |
...
- **"Models" and "warehouse" over "platform."** Kilo is not a SaaS dashboard. It's a warehouse generator.
